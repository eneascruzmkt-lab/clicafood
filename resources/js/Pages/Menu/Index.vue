...
const onDragEnd = () => {
    ...
    router.post('/menu-items/reorder', { ids }, { preserveScroll: true });
};
</script>
    ...
    <AppLayout title="Cardápio">
        <div class="flex items-center justify-between mb-8">
            <div>
                ...
                <p class="text-dark-400 mt-1">Arraste para reordenar os itens</p>
            </div>
            ...
            <template #item="{ element: item }">
                <div class="card flex items-center gap-4 hover:border-dark-600 transition-colors">
                    ...
                    <div class="w-16 h-16 rounded-lg overflow-hidden flex-shrink-0 bg-dark-700 relative">
                        <img v-if="item.image" :src="mediaUrl(item.image)" :alt="item.name" class="w-full h-full object-cover" />
                        <div v-else class="w-full h-full flex items-center justify-center">
                            <Icon name="image" class="w-6 h-6 text-dark-500" />
                        </div>
                        <div v-if="item.video_url" class="absolute bottom-0.5 right-0.5 bg-brand-500 rounded-full w-5 h-5 flex items-center justify-center">
                    ...
                    <div class="flex-1 min-w-0">
                        <div class="flex items-center gap-2">
                            ...
                            <span v-if="item.featured" class="inline-flex items-center gap-1 px-2 py-0.5 bg-amber-500/15 text-amber-400 text-xs rounded-full">
                    ...
                    <div class="flex items-center gap-1">
                        <Link :href="`/menu-items/${item.id}/edit`" class="text-dark-400 hover:text-white transition-colors p-2">
                            <Icon name="edit" class="w-5 h-5" />
                        </Link>
                        <button @click="deleteItem(item.id)" class="text-dark-400 hover:text-red-400 transition-colors p-2">
